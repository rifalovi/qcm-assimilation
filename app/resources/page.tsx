...
const OFFICIAL_LINKS = {
  ...
  examInfo: "https://www.immigration.interieur.gouv.fr/Immigration/Examen-civique-pour-une-premiere-demande-de-titre-de-sejour-pluriannuel",
  officialQuestions: "https://www.immigration.interieur.gouv.fr/Integration-et-Acces-a-la-nationalite/La-nationalite-francaise/Les-procedures-d-acces-a-la-nationalite-francaise",
  charter: "https://www.prefecturedepolice.interieur.gouv.fr/sites/default/files/Documents/Chartedesdroitsetdevoirs.pdf",
};
...
function PathStep({ step, title, text, accent = "blue" }: {
...
function FeatureCard({ icon, title, description, href, cta, featured = false, badge }: {
...
function ResourceLinkCard({ icon, title, source, text, href, note }: {
...
function AccountModal({ open, onClose, onTest }: { open: boolean; onClose: () => void; onTest: () => void }) {
  if (!open) return null;
  return (
    ...
    <>
      <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
        <div className="space-y-8 sm:space-y-10">
            ...
            <div className="relative px-5 py-7 sm:px-8 sm:py-9">
              ...
              <div className="text-center">
                ...
                <div className="mt-7 flex flex-col items-center justify-center gap-3 sm:flex-row sm:flex-wrap">
                  <HeroButton onClick={() => router.push("/scroll")} variant="primary">🚀 Réviser en scroll</HeroButton>
                  <HeroButton onClick={() => router.push("/audio")} variant="secondary">🎧 Bibliothèque audio</HeroButton>
                  <HeroButton onClick={() => router.push("/quiz")} variant="secondary">🎯 Faire un test</HeroButton>
                </div>
              </div>
            </div>
          </section>
          ...
          {/* ── CHECKLIST ── */}
          <SectionCard icon="✅" title="Checklist pour bien réussir" subtitle="Avant de penser au centre d'examen, assurez-vous d'avoir posé les bonnes bases.">
            <div className="grid gap-4 md:grid-cols-2">
              <ChecklistItem title="Commencer par un test" text="Faites une première session pour repérer rapidement votre niveau et identifier les thèmes qui vous posent le plus de difficultés." />
              <ChecklistItem title="Réviser régulièrement en scroll" text="Utilisez la révision courte et fréquente pour ancrer les notions sans vous fatiguer, en particulier sur les thèmes que vous maîtrisez moins." />
              <ChecklistItem title="Écouter les épisodes audio" text="Utilisez la bibliothèque audio pour réviser en déplacement — chaque épisode couvre un sous-thème en format entretien réel, voix naturelle." />
              <ChecklistItem title="Lire les explications après chaque erreur" text="Ne vous contentez pas de corriger : prenez le temps de comprendre pourquoi la bonne réponse est correcte." />
              <ChecklistItem title="Suivre ses résultats" text="Consultez vos scores, vos erreurs fréquentes et vos progrès pour ajuster votre stratégie de révision." />
              <ChecklistItem title="Revenir sur les thèmes faibles" text="Une progression réelle vient souvent d'un travail ciblé sur 1 ou 2 thèmes prioritaires, pas d'une révision dispersée." />
            </div>
          </SectionCard>

          {/* ── PARCOURS ── */}
          <SectionCard icon="🧭" title="Le parcours conseillé sur la plateforme" subtitle="Une méthode simple pour tirer le meilleur parti de l'application.">
            <div className="grid gap-4 lg:grid-cols-4">
              <PathStep step="1" title="Découvrir" text="Lisez la page info pour comprendre le cadre général, puis faites un premier test sans pression." accent="blue" />
              <PathStep step="2" title="Diagnostiquer" text="Analysez vos résultats pour repérer vos thèmes faibles et les types de questions qui vous bloquent." accent="violet" />
              <PathStep step="3" title="Réviser" text="Travaillez vos thèmes prioritaires en scroll et en audio pour progresser plus vite et mémoriser durablement." accent="amber" />
              <PathStep step="4" title="Valider" text="Refaites un test ou un examen blanc, puis utilisez les ressources externes comme étape suivante." accent="emerald" />
            </div>
          </SectionCard>

          {/* ── CARTE MENTALE ── */}
          <SectionCard icon="🧠" title="Carte mentale de préparation" subtitle="Une vue simple des grands leviers pour progresser efficacement.">
            <div className="grid gap-4 lg:grid-cols-3">
              <MindNode title="1. Comprendre" items={["Savoir ce qu'on attend de vous", "Repérer les thèmes importants", "Se familiariser avec la logique des questions"]} />
              <MindNode title="2. S'entraîner" items={["Faire un test global", "Identifier ses faiblesses", "Mesurer son niveau de départ"]} />
          ...
          {/* ── OUTILS ── */}
          <SectionCard icon="📱" title="Les meilleurs points d'entrée dans l'application" subtitle="Conservez une logique simple : d'abord l'apprentissage, ensuite l'action.">
            <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
              <FeatureCard icon="📱" title="Réviser en scroll" description="Le mode le plus immersif pour apprendre rapidement, réviser souvent et revenir facilement sur un thème précis." href="/scroll" cta="Ouvrir la révision" featured />
              <FeatureCard icon="🎧" title="Bibliothèque audio" description="100 épisodes guidés en voix naturelle. Révisez en déplacement, en cuisine, partout — format entretien réel." href="/audio" cta="Écouter maintenant" badge="Nouveau" />
              <FeatureCard icon="🎯" title="Faire un test" description="Le meilleur point de départ si vous voulez mesurer votre niveau actuel et savoir où concentrer vos efforts." href="/quiz" cta="Commencer un test" />
              <FeatureCard icon="📊" title="Voir mes résultats" description="La bonne page pour comprendre vos progrès, revoir vos erreurs et décider quoi réviser ensuite." href="/results" cta="Accéder aux résultats" />
            </div>
      ...
      <AccountModal open={showAccountModal} onClose={() => setShowAccountModal(false)} onTest={() => { setShowAccountModal(false); router.push("/quiz"); }} />
