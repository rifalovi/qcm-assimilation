...
function StarPicker({ value, onChange, label }: { value: number; onChange: (v: number) => void; label: string }) {
  ...
  return (
    <div>
      <p className="text-sm mb-2" style={{ color: "var(--cc-text-muted)" }}>{label}</p>
      <div className="flex gap-1">
        {[1,2,3,4,5].map((i) => (
          <button key={i} type="button" onClick={() => onChange(i)}
            onMouseEnter={() => setHovered(i)} onMouseLeave={() => setHovered(0)}
            className="transition-transform hover:scale-110">
            <Star size={28} className={i <= (hovered || value) ? 'text-amber-400 fill-amber-400' : 'opacity-30'} style={i <= (hovered || value) ? {} : { color: "var(--cc-text-muted)" }} />
          </button>
        ))}
      </div>
      {value > 0 && <p className="text-xs mt-1" style={{ color: "var(--cc-text-disabled)" }}>{['','Très mauvais','Mauvais','Correct','Bien','Excellent'][value]}</p>}
    </div>
  )
}
...
function QuestionPicker({ value, onChange }: { value: string[]; onChange: (v: string[]) => void }) {
  ...
  return (
    <div>
      <p className="text-sm mb-1" style={{ color: "var(--cc-text-muted)" }}>Questions posées lors de votre passage</p>
      <p className="text-xs mb-3" style={{ color: "var(--cc-text-disabled)" }}>Sélectionnez ou ajoutez (max 10)</p>
      {value.length > 0 && (
      ...
      <div className="flex flex-wrap gap-1.5 mb-3">
        {SUGGESTIONS.filter((q) => !value.includes(q)).map((q) => (
          <button key={q} type="button" onClick={() => toggle(q)} disabled={value.length >= 10}
            className="text-xs px-2.5 py-1 rounded-full border disabled:opacity-40 transition-colors hover:opacity-80"
            style={{ background: "var(--cc-surface-alt)", color: "var(--cc-text-muted)", borderColor: "var(--cc-border)" }}>
            + {q}
          </button>
        ))}
      </div>
      <div className="flex gap-2">
        <input type="text" value={custom} onChange={(e) => setCustom(e.target.value)}
          ...
          className="flex-1 text-xs rounded-xl px-3 py-2 border focus:outline-none focus:ring-2 focus:ring-teal-500/30 focus:border-teal-400"
...
export default function NewTestimonyPage() {
  ...
  if (submitted) return (
    <main className="max-w-2xl mx-auto px-4 py-16 text-center">
      ...
      <h1 className="text-xl font-medium mb-2" style={{ color: "var(--cc-text)" }}>Merci pour votre témoignage !</h1>
      <p className="text-sm mb-6" style={{ color: "var(--cc-text-muted)" }}>Votre retour d&apos;expérience aidera d&apos;autres candidats à se préparer.</p>
  ...
  return (
    <main className="max-w-2xl mx-auto px-4 py-8" style={{ color: "var(--cc-text)" }}>
        ...
        <>
          <div className="flex items-center gap-2 mb-8">
            {[1,2,3].map((s) => (
              <div key={s} className="flex items-center gap-2">
                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-medium transition-colors ${s < step ? 'bg-teal-600 text-white' : s === step ? 'bg-white text-slate-900' : ''}`} style={s >= step && s !== step ? { background: "var(--cc-surface-alt)", color: "var(--cc-text-disabled)" } : {}}>
                  {s < step ? <Check size={13} /> : s}
                </div>
                {s < 3 && <div className={`h-0.5 w-8 ${s < step ? 'bg-teal-600' : ''}`} style={s >= step ? { background: "var(--cc-border)" } : {}} />}
              </div>
            ))}
            <span className="text-xs ml-2" style={{ color: "var(--cc-text-disabled)" }}>Étape {step}/3</span>
          </div>

          {step === 1 && (
            <div className="space-y-5">
              <div><h1 className="text-xl font-medium mb-1" style={{ color: "var(--cc-text)" }}>Votre passage</h1><p className="text-sm" style={{ color: "var(--cc-text-muted)" }}>Informations générales sur votre expérience</p></div>
              <div>
                <p className="text-sm mb-2" style={{ color: "var(--cc-text-muted)" }}>Type de passage <span className="text-red-400">*</span></p>
                <div className="grid grid-cols-2 gap-3">
                  {[{value:'test_civique',label:'Test civique',desc:'Test de connaissance'},{value:'entretien_naturalisation',label:'Entretien',desc:'Entretien de naturalisation'}].map(({value,label,desc}) => (
                    <button key={value} type="button" onClick={() => update('type', value as TestimonyType)}
                      className={`p-4 rounded-2xl border text-left transition-colors ${form.type === value ? 'border-teal-300 bg-teal-50' : 'hover:opacity-80'}`}
                      style={form.type !== value ? { borderColor: "var(--cc-border)", background: "var(--cc-surface-alt)" } : {}}>
                      <p className="text-sm font-medium" style={{ color: "var(--cc-text)" }}>{label}</p>
                      <p className="text-xs mt-0.5" style={{ color: "var(--cc-text-disabled)" }}>{desc}</p>
                    </button>
                  ))}
                </div>
              </div>
              <div>
                <p className="text-sm mb-2" style={{ color: "var(--cc-text-muted)" }}>Résultat</p>
                <div className="flex gap-2">
                  {[{value:true,label:'✓ Réussi',color:'border-emerald-200 bg-emerald-50 text-emerald-700'},{value:false,label:'✗ Non réussi',color:'border-red-200 bg-red-50 text-red-600'}].map(({value,label,color}) => (
                    <button key={String(value)} type="button" onClick={() => update('passed', form.passed === value ? null : value)}
                      className={`flex-1 py-2.5 rounded-xl border text-sm font-medium transition-colors ${form.passed === value ? color : ''}`}
                      style={form.passed !== value ? { borderColor: "var(--cc-border)", background: "var(--cc-surface-alt)", color: "var(--cc-text-muted)" } : {}}>
                      {label}
                    </button>
                  ))}
                </div>
              ...
              <div>
                <label className="block text-sm mb-1.5" style={{ color: "var(--cc-text-muted)" }}>Ville du passage</label>
              ...
              <div>
                <label className="block text-sm mb-1.5" style={{ color: "var(--cc-text-muted)" }}>Date du passage</label>
          ...
          {step === 2 && (
            <div className="space-y-6">
              <div><h1 className="text-xl font-medium mb-1" style={{ color: "var(--cc-text)" }}>Notes & questions</h1><p className="text-sm" style={{ color: "var(--cc-text-muted)" }}>Ces informations sont optionnelles mais très utiles</p></div>
          ...
          {step === 3 && (
            <div className="space-y-5">
              <div><h1 className="text-xl font-medium mb-1" style={{ color: "var(--cc-text)" }}>Votre témoignage</h1><p className="text-sm" style={{ color: "var(--cc-text-muted)" }}>Racontez votre expérience — déroulement, ambiance, conseils…</p></div>
              <div>
                ...
                <div className="flex justify-between mt-1">
                  <p className={`text-xs ${form.free_text.length < 20 ? 'text-red-400' : ''}`} style={form.free_text.length >= 20 ? { color: "var(--cc-text-muted)" } : {}}>
                    {form.free_text.length < 20 ? `Minimum 20 caractères (${20 - form.free_text.length} restants)` : ''}
                  </p>
                  <p className="text-xs" style={{ color: "var(--cc-text-disabled)" }}>{form.free_text.length}/2000</p>
                </div>
              </div>
              <div className="rounded-2xl p-4 space-y-1.5 border" style={{ borderColor: "var(--cc-border)", background: "var(--cc-surface-alt)" }}>
                <p className="text-xs font-medium mb-2" style={{ color: "var(--cc-text-muted)" }}>Récapitulatif</p>
                <p className="text-xs" style={{ color: "var(--cc-text-muted)" }}><span style={{ color: "var(--cc-text-disabled)" }}>Type : </span>{form.type === 'test_civique' ? 'Test civique' : 'Entretien de naturalisation'}</p>
                {form.city && <p className="text-xs" style={{ color: "var(--cc-text-muted)" }}><span style={{ color: "var(--cc-text-disabled)" }}>Ville : </span>{form.city}</p>}
                {form.passed !== null && <p className="text-xs" style={{ color: "var(--cc-text-muted)" }}><span style={{ color: "var(--cc-text-disabled)" }}>Résultat : </span>{form.passed ? '✓ Réussi' : '✗ Non réussi'}</p>}
                {form.questions_asked.length > 0 && <p className="text-xs" style={{ color: "var(--cc-text-muted)" }}><span style={{ color: "var(--cc-text-disabled)" }}>Questions : </span>{form.questions_asked.length} sélectionnée{form.questions_asked.length > 1 ? 's' : ''}</p>}
              </div>
            </div>
          )}
          ...
          <div className="mt-8">
            {step < 3 ? (
              <button type="button" onClick={() => setStep(step + 1)} disabled={!canProceed()}
                className="w-full flex items-center justify-center gap-2 text-sm font-medium py-3 rounded-xl disabled:opacity-40 transition-colors hover:opacity-90"
