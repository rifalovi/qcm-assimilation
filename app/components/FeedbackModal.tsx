...
export default function FeedbackModal({
  ...
  const resolvedPseudo = (username || pseudo || "Anonyme").trim();
  const resolvedEmail = (authEmail || email || "").trim();

  if (!open) return null;

  if (!isLoggedIn) {
    return (
      <div className="fixed inset-0 z-[200] flex items-end justify-center px-4 pb-4 sm:items-center">
        <div
          ...
          <div className="p-6 text-center">
            ...
            <div className="mt-4 flex flex-col gap-2">
              <a
                href="/register"
                onClick={onClose}
                className="block w-full rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 px-4 py-2.5 text-xs font-bold text-white transition hover:brightness-110"
              >
            ...
            <>
              ...
              <div className="mt-3 flex gap-2">
                <button
                  ...
                  className="flex-1 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 px-3 py-2.5 text-xs font-bold text-white shadow-[0_8px_24px_rgba(37,99,235,0.3)] transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-30 active:scale-[0.98]"
                >
