  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: resultsData } = await supabase
        .from("results")
        .select(
          "id, score_correct, score_total, score_percent, passed, level, themes, mode, created_at"
        )
        .eq("email", user.email)
            ...
            <div className="flex items-start justify-between gap-4 flex-wrap">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <span
                    className={`inline-flex items-center gap-2 rounded-full border px-2.5 py-1 text-xs font-semibold ${roleConfig.color}`}
                  >
        ...
        {quickStats && (
          <section className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <StatCard
              ...
              value={String(quickStats.totalTests)}
              accent="blue"
            />
            <StatCard
              label="Meilleur score"
              value={quickStats.bestScore}
              accent="emerald"
            />
            <StatCard
              label="Tests réussis"
              value={String(quickStats.passedCount)}
              accent="amber"
            />
          </section>
        )}
                  ...
                  <div className="flex min-w-0 items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="truncate text-xs text-slate-400">
                        Niveau {r.level} •{" "}
                        {Array.isArray(r.themes) ? r.themes.join(", ") : r.themes}
                      </p>
                    </div>
                  </div>
