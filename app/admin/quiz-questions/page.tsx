    ...
    <div className="space-y-6">
      ...
      {stats && (
        <div className="grid gap-3 sm:grid-cols-5">
          {[
            { l: "Niveau 1", v: stats.level1, c: "border-[var(--cc-primary)] bg-[var(--cc-info-soft)] text-[var(--cc-primary)]" },
            { l: "Niveau 2", v: stats.level2, c: "border-[var(--cc-primary)] bg-[var(--cc-primary-soft)] text-[var(--cc-primary)]" },
            { l: "Niveau 3", v: stats.level3, c: "border-[var(--cc-warning)] bg-[var(--cc-warning-soft)] text-[var(--cc-warning)]" },
            { l: "Actives", v: stats.active, c: "border-[var(--cc-success)] bg-[var(--cc-success-soft)] text-[var(--cc-success)]" },
      ...
      {/* Import initial */}
      {total === 0 && !loading && (
        <div className="rounded-2xl border border-[var(--cc-primary)] bg-[var(--cc-primary-soft)] p-4 flex items-center justify-between flex-wrap gap-3">
          <div>
            <p className="text-sm font-bold text-[var(--cc-primary)]">Base vide — Importer les QCM existants</p>
            <p className="text-xs text-[var(--cc-primary)] mt-0.5">458 QCM sont définis dans les fichiers statiques. Cliquez pour les importer en base.</p>
          </div>
            ...
            <div key={q.id} className="adm-panel p-4">
              <div className="flex items-start justify-between gap-3 flex-wrap">
                <div className="flex-1 min-w-0">
                  ...
                  {q.answer && (
                    <p className="text-xs text-[var(--cc-success)] mt-1">
          ...
          <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto adm-panel p-6 space-y-4">
            ...
            <div>
              ...
              <input value={editing.external_id ?? ""} onChange={e => setEditing({ ...editing, external_id: e.target.value || null })}
                className="w-full rounded-xl border border-[var(--cc-border)] bg-[var(--cc-surface)] px-3 py-2 text-sm focus:outline-none font-mono"
                style={{ color: 'var(--cc-text)' }} />
            </div>
            <div>
              ...
              <textarea value={editing.question ?? ""} onChange={e => setEditing({ ...editing, question: e.target.value })}
                className="w-full min-h-[80px] rounded-xl border border-[var(--cc-border)] bg-[var(--cc-surface)] px-3 py-2 text-sm focus:outline-none"
                style={{ color: 'var(--cc-text)' }} />
            </div>
            {(["a", "b", "c", "d"] as const).map(k => (
              <div key={k} className="flex gap-2">
                <div className="w-8 h-8 flex items-center justify-center rounded-lg bg-[var(--cc-surface-raised)] text-sm font-bold" style={{ color: 'var(--cc-text)' }}>{k.toUpperCase()}</div>
                <input value={editing[`choice_${k}` as "choice_a"] ?? ""} onChange={e => setEditing({ ...editing, [`choice_${k}`]: e.target.value })}
                  className="flex-1 rounded-xl border border-[var(--cc-border)] bg-[var(--cc-surface)] px-3 py-2 text-sm focus:outline-none" placeholder={`Option ${k.toUpperCase()}`}
                  style={{ color: 'var(--cc-text)' }} />
              </div>
            ))}
            <div>
              ...
              <div className="flex gap-2">
                {(["A","B","C","D"] as const).map(k => (
                  <button key={k} onClick={() => setEditing({ ...editing, answer: k })}
                    className={`flex-1 rounded-xl border px-3 py-2 text-sm font-bold ${editing.answer === k ? "border-[var(--cc-success)] bg-[var(--cc-success-soft)] text-[var(--cc-success)]" : "border-[var(--cc-border)] bg-[var(--cc-surface)]"}`}
            ...
            <div>
              ...
              <textarea value={editing.explanation ?? ""} onChange={e => setEditing({ ...editing, explanation: e.target.value })}
                className="w-full min-h-[60px] rounded-xl border border-[var(--cc-border)] bg-[var(--cc-surface)] px-3 py-2 text-sm focus:outline-none"
                style={{ color: 'var(--cc-text)' }} />
            </div>
