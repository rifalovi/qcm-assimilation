...
export default function ResultsPage() {
  ...
  // Charger pseudo (pour l’affichage)
  useEffect(() => {
  ...
  // 2) Charger le dernier résultat (lié à l’email + mode)
  useEffect(() => {
    if (typeof window === "undefined") return;
    ...
    // ✅ clé finale attendue : last_result:train:email OU last_result:exam:email
  ...
  // Auto-scroll hash feedback (sans casser SSR)
  useEffect(() => {
    if (typeof window === "undefined") return;
  ...
  function mailResult() {
    ...
    // 🔒 anti “speed-run” : au moins 32 réponses cochées
...
— QCM Assimilation FR`
  ...
  // ✅ ICI : TON DESIGN / JSX RESTE IDENTIQUE
  return (
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          <span
            className={`px-3 py-1 rounded-full text-sm font-semibold border ${
              score.passed ? "bg-green-50 text-green-700 border-green-200" : "bg-red-50 text-red-700 border-red-200"
            }`}
          >
        ...
        <div className="mt-6 grid grid-cols-10 gap-2">
          {data.questions.map((q, i) => {
            ...
            return (
              <div
                key={q.id}
                className={`h-6 w-6 rounded-md transition ${isWrong ? "bg-red-500" : "bg-green-500"}`}
                ...
                {[1, 2, 3, 4, 5].map((n) => (
                  <button
                    ...
                    className={`h-10 w-10 rounded-xl border font-semibold transition ${
                      rating === n ? "border-blue-600 bg-blue-50 text-blue-700" : "border-slate-200 hover:bg-slate-50"
                    }`}
                  >
                    {n}
                  </button>
            ...
            {wrong.slice(0, 20).map((d, i) => (
              <div key={d.id} className="rounded-2xl border border-slate-200 p-4 bg-white">
                ...
                <div className="mt-3 text-sm">
                  <span className="font-semibold">Ta réponse :</span> {d.user ? `${d.user}` : "— (non répondu)"}
                </div>
                ...
                <div className="mt-3 text-sm text-slate-700">
                  <span className="font-semibold">Explication :</span> {d.explanation}
                </div>
              </div>
            ))}
          </div>
