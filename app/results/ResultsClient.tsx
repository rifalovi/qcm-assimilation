...
export default function ResultsClient() {
  ...
  useEffect(() => {
    ...
    async function fetchResult() {
      ...
      // Toujours essayer last_result comme fallback (anonyme ou email non encore chargé)
...
— Cap Citoyen`
  ...
  <div className="mt-6 grid grid-cols-4 gap-3 sm:grid-cols-8">
    <button onClick={replaySame}
      className="flex flex-col items-center gap-1.5 rounded-2xl border border-blue-400/20 bg-blue-500/8 px-2 py-3 text-blue-400 transition hover:bg-blue-500/15 active:scale-95">
    ...
    <button onClick={() => router.push(mode === "exam" ? "/exam" : "/")}
      className="flex flex-col items-center gap-1.5 rounded-2xl border border-indigo-400/20 bg-indigo-500/8 px-2 py-3 text-indigo-400 transition hover:bg-indigo-500/15 active:scale-95">
